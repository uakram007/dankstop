    ...
    var e, i, o = {
            ...
            customEvents:function(){ 
                ...
                setTimeout(function() {
                    ...
                    console.log(['https://dankstop.com','https://dankstop.com/'].indexOf(window.location.href) > -1)
                    if(['https://dankstop.com','https://dankstop.com/'].indexOf(window.location.href) > -1 != true){
					    window.dataLayer.push({
						    event: "page_viewed", 
                            page_location: window.location.href, // 👉 Correct URL
                ...
                setTimeout(() => {
                    if(window.location.href.indexOf('collections/') > -1){
                        ...
                        let collection_id = ''
                        let collection_name = ''
                        document.querySelectorAll('.product-collection .product-item').forEach(function(element) { 
                        ...
                        if (scriptTag.length > 0) { 
                            ...
                            collection_view_event_elements.push({...scriptContent});
                        }
                        });
                    ...
                    if(window.location.href.indexOf('products/') > -1 ){
                        let selected_variant = document.querySelector('#product-selectors').value
                        let matchingElement = document.querySelector(`#cc-pricing-variants-select option[value="${selected_variant}"]`); 
                        if (!matchingElement) {
                            console.error('No matching element found for the selected variant');
                            return;
                        }else{
                            let view_item_event = JSON.parse(document.querySelector('.view__item').innerHTML)
                            window.dataLayer = window.dataLayer || [];
                            window.dataLayer.push({ ecommerce: null });
                            view_item_event.event = "view_item";
                            view_item_event.ecommerce.value = parseFloat(matchingElement.getAttribute('data-price').trim());
                            view_item_event.ecommerce.items[0].item_variant = matchingElement.textContent.trim() == "Default Title" ? "Single Variant" : matchingElement.textContent.trim();
                            view_item_event.ecommerce.items[0].price = parseFloat(matchingElement.getAttribute('data-price').trim());
                            window.dataLayer.push(view_item_event);
                        } 
                    }
                    ...
                    if(window.location.href.indexOf('/cart') > -1 ){
                        ...
                        let currency = ''
                        let cart_total = 0
                        document.querySelectorAll('.new_event_cart').forEach(function(element) {
                            var scriptTag = $(element);
                            if (scriptTag.length > 0) {  
                                var scriptContent = JSON.parse(scriptTag.html());  
                                currency = element.getAttribute('data-currency') || '';
                                cart_total = element.getAttribute('data-cart-total') || '';
                                cart_items.push(scriptContent);
                            }
                        });
                        window.dataLayer.push({
                            event: "view_cart", 
                            ecommerce: {
                                currency,
                                value: parseFloat(cart_total.replace(/,/g, '')),
                                items: cart_items
                            }
                        });
                        ...
                        document.querySelectorAll('button.update.btn').forEach(function(element) {
                            element.addEventListener('click', function(e) {  
                                window.dataLayer = window.dataLayer || [];
                                window.dataLayer.push({ ecommerce: null });

                                let quantity = e.target.closest('.quantity') 
                                let current_value = quantity.querySelector('.qty-group input').value
                                let old_value = quantity.querySelector('.qty-group input').getAttribute('data-old-value')
                                let pid = quantity.querySelector('.qty-group input').getAttribute('data-pid')
                                let script_data = document.querySelector(`#new_event_cart_${pid}`)
                                if(current_value > old_value){
                                    window.dataLayer.push({
                                        'event': 'add_to_cart',
                                        'ecommerce': {
                                            currency: script_data.getAttribute('data-currency'),
                                            value: JSON.parse(script_data.innerHTML).price * (current_value-old_value),
                                            'items': [{...JSON.parse(script_data.innerHTML), quantity: current_value-old_value }]
                                        }
                                    });
                                    ...
                                    window.dataLayer.push({
                                        ...
                                        'ecommerce': {
                                            currency: script_data.getAttribute('data-currency'),
                                            value: JSON.parse(script_data.innerHTML).price * (current_value-old_value),
                                            'items': [{...JSON.parse(script_data.innerHTML), quantity: old_value-current_value }]
                                        }
                                    });
                                }
                            })
                        ...
                        document.querySelectorAll('.remove').forEach(function(element) {
                            element.addEventListener('click', function(e) {  
                                ...
                                window.dataLayer.push({
                                    ...
                                    'ecommerce': {
                                        currency: script_data.getAttribute('data-currency'),
                                        value:JSON.parse(script_data.innerHTML).price * JSON.parse(script_data.innerHTML).quantity,
                                        'items': [JSON.parse(script_data.innerHTML)]
                                    }
                                });
                            })
                        })
            ...
            checkPrices:function(){ 
                document.querySelectorAll('.v-id').forEach((labelElement) => {
                    labelElement.addEventListener('click', () => {  
                        setTimeout(()=>{
                            let activeVariant = ''
                            document.querySelectorAll('.swatch-element input[type="radio"]:checked').forEach( activeBtn=>{
                            activeVariant += activeBtn.value + ' / '
                            }) 
                            activeVariant = activeVariant.trim().slice(0, -1) 
                            checkAndUpdatePrices(activeVariant) 
                        },10)
                        
                    });
                });

                function checkAndUpdatePrices(clickedValue){
                    // console.log("Clicked value is:", clickedValue); 
                    let variantElements = document.querySelector('#cc-pricing-variants-select')
                    let matchingElement;
                    Array.from(variantElements.options).forEach(function(option_element) {
                        let ve = option_element.text.toLocaleLowerCase().trim();
                        let cv = clickedValue.toLocaleLowerCase().trim() 
                        if ( ve == cv ) {
                            matchingElement = option_element;  
                        }
                    });
                ...
                function updatePricesFromVariant(matchingElement) {
                    ...
                    // Show the cells if a match is found
                    if (memberCell) memberCell.style.display = '';
                    if (eliteCell) eliteCell.style.display = '';
                    marketCell.style.display = '';		
                    // btnATC.style.display = '';		
                    // Get data-m and data-e from the matching variant element
                    const memberPrice = matchingElement.getAttribute('data-member-price');
                    const elitePrice = matchingElement.getAttribute('data-elite-price');
                    const marketPrice = matchingElement.getAttribute('data-price'); 
            
                    // Update Member price
                    if (memberCell) {
                        const memberPriceSpan = memberCell.querySelector('.price_new');
                        if (memberPriceSpan && memberPrice !== null && memberPrice != '0.0') {
                            memberPriceSpan.textContent = `$${parseFloat(memberPrice).toFixed(2)}`;
                        } else if (memberPriceSpan) { 
                            memberCell.style.display = 'none';
                            eliteCell.style.display = 'none';
                            marketCell.style.display = 'none';
                            // btnATC.style.display = 'none';
                        }
                    }
                    ...
                    // Update Elite price
                    if (eliteCell) {
                        const elitePriceSpan = eliteCell.querySelector('.price_new');
                        if (elitePriceSpan && elitePrice !== null && elitePrice != '0.0') {
                            elitePriceSpan.textContent = `$${parseFloat(elitePrice).toFixed(2)}`;
                        } else if (elitePriceSpan) { 
                            memberCell.style.display = 'none';
                            eliteCell.style.display = 'none';
                            marketCell.style.display = 'none';
                            // btnATC.style.display = 'none';
                    ...
                    if (marketCell) {
                        const marketPriceSpan = marketCell.querySelector('.price_new');
                        if (marketPriceSpan && marketPrice !== null  && elitePrice != '0.0' && memberPrice != '0.0') {
                            marketPriceSpan.textContent = `$${marketPrice}`;
                        } else if (marketPriceSpan) {
                            memberCell.style.display = 'none';
                            eliteCell.style.display = 'none';
                            marketCell.style.display = 'none';
                            // btnATC.style.display = 'none';
                    ...
                    // console.log(`Updated prices: Member - $${memberPrice || "N/A"}, Elite - $${elitePrice || "N/A"}`);
                }
            },
            checkPOPSSOUP:function(){ 
              ...
              eliteUpgradeBtn.forEach((btn) => {
                ...
                //       `${SSU_URL}/popup/${path}?client=${client}&return_to=${safeBase64Url}&parentOrigin=${encodeURIComponent(window.location.origin)}`, // Replace with your SSO URL
                //       "SSO_Login",
            ...
            checkElite:function(){
                ...
                //         var itemInCart = cart.items.find(item => item.variant_id == '46302742249691');
            ...
            initAddToCart: function() {
                console.log("initAddToCart loadeddd...yy");
                ...
                s.off("click.addToCart", e).on("click.addToCart", e, function(e) {
                    ...
                    if(window.location.href.indexOf('products/') > -1 ){ 
                        ...
                        if (!matchingElement) {
                            ...
                            let add_to_cart_event = JSON.parse(document.querySelector('.view__item').innerHTML)
                            window.dataLayer = window.dataLayer || [];
                            window.dataLayer.push({ ecommerce: null });
                            add_to_cart_event.event = "add_to_cart";
                            add_to_cart_event.ecommerce.value = parseFloat(matchingElement.getAttribute('data-price').trim()) * parseFloat(document.querySelector('input#quantity__atc').value.trim());
                            add_to_cart_event.ecommerce.items[0].quantity = parseFloat(document.querySelector('input#quantity__atc').value.trim());
                            add_to_cart_event.ecommerce.items[0].item_variant = matchingElement.textContent.trim() == "Default Title" ? "Single Variant" : matchingElement.textContent.trim();
                            add_to_cart_event.ecommerce.items[0].price = parseFloat(matchingElement.getAttribute('data-price').trim());
                    ...
                    if ("disabled" !== i.attr("disabled")) {
                        ...
                        switch (l || (l = 1), window.ajax_cart) {
                            ...
                            case "normal":
                                ...
                                break;
                            case "upsell":
                                let ePro = document.querySelector("#accept_policy_elite");
                               if(ePro && ePro.checked){
                                 o.doAjaxAddToCartNormal(ePro.value, 1, d);
                                 setTimeout(function(){
                                    document.querySelector(".cc-pricing-table-member-cell-inner").style.display = "none";
